...
import { initializeSugarRush } from './sugarRush'
import { initializeMilestones } from './milestone'
import { initializeTowerDynamics } from './towerDynamics'
import { cakeStyles, initializeCakeCollection } from './cakeCollection'
import {
  initializeRunPrompts,
  addRunPromptInstance
} from './runPrompt'
...
window.TowerGame = (option = {}) => {
  ...
  const pathGenerator = (path) => {
    const embeddedAssets = typeof window !== 'undefined'
      ? window.SWEET_STACK_ASSETS
      : null
    return embeddedAssets && embeddedAssets[path]
      ? embeddedAssets[path]
      : `./assets/${path}`
  }

  game.addImg('background', pathGenerator('background.png'))
  game.addImg('background-extension', pathGenerator('background-extension.png'))
  game.addImg('background-extension-sunset', pathGenerator('background-extension-sunset.png'))
  game.addImg('background-extension-night', pathGenerator('background-extension-night.png'))
  ...
  cakeStyles.forEach((style) => {
    game.addImg(style.id, pathGenerator(`cakes/${style.id}.png`))
  })
  for (let i = 1; i <= 8; i += 1) {
  ...
  game.swapLayer(0, 1)
  game.addLayer(constant.feedbackLayer)
  ...
  game.addAudio('drop-safe', pathGenerator('drop-safe.wav'))
  game.addAudio('sugar-rush', pathGenerator('sugar-rush.wav'))
  ...
  // Keep the swing arc consistent on tall phones. The original height-only
  // formula makes a portrait phone's rope several cake-widths longer than it
  // is on desktop, which drastically shortens the timing window.
  game.setVariable(
    constant.ropeHeight,
    Math.min(game.height * 0.4, game.width * 0.72)
  )
  ...
  initializeSugarRush(game)
  initializeMilestones(game)
  initializeRunPrompts(game)
  initializeTowerDynamics(game)
  initializeCakeCollection(game)
  for (let i = 1; i <= 4; i += 1) {
